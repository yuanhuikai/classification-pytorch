...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if type(self.attn_mask) != type(None):
            attn_mask = torch.tensor(self.attn_mask).to(
                x_windows.device) if x.is_cuda else torch.tensor(self.attn_mask)
        else:
            attn_mask = None
        # nW*B, window_size*window_size, C
...
def swin_transformer_tiny(input_shape=[224, 224], pretrained=False, num_classes=1000):
    model = SwinTransformer(input_shape, depths=[2, 2, 6, 2], num_heads=[
                            3, 6, 12, 24], embed_dim=96, drop_path_rate=0.2)
    if pretrained:
        model.load_state_dict(torch.load(
            "model_data/swin_tiny_patch4_window7_224_imagenet1k.pth"))
...
def swin_transformer_small(input_shape=[224, 224], pretrained=False, num_classes=1000):
    model = SwinTransformer(input_shape, depths=[2, 2, 18, 2], num_heads=[
                            3, 6, 12, 24], embed_dim=96, drop_path_rate=0.3)
    if pretrained:
        model.load_state_dict(torch.load(
            "model_data/swin_small_patch4_window7_224_imagenet1k.pth"))
...
def swin_transformer_base(input_shape=[224, 224], pretrained=False, num_classes=1000):
    model = SwinTransformer(input_shape, depths=[2, 2, 18, 2], num_heads=[
                            4, 8, 16, 32], embed_dim=128, drop_path_rate=0.5)
    if pretrained:
        model.load_state_dict(torch.load(
            "model_data/swin_base_patch4_window7_224_imagenet1k.pth"))
